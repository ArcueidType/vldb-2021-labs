...
func (rl *ResolveLock) PrepareWrites(txn *mvcc.MvccTxn) (interface{}, error) {
	...
	for _, kl := range rl.keyLocks {
		...
		if commitTs == 0{
			rollback_response, err := rollbackKey(kl.Key, txn, response)
			if err != nil{
				return rollback_response, err
			}
		} else {
			commit_response, err := commitKey(kl.Key, commitTs, txn, response)
			if err != nil{
				return commit_response, err
			}
		}
	}
